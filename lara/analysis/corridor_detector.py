...
class CorridorDetector:
    ...
    def detect_corridors(self, grid_size_km: float = DEFAULT_GRID_SIZE_KM,
                        min_flights: int = MIN_CORRIDOR_FLIGHTS) -> Dict[str, Any]:
        """
        ...
        for coords, data in grid.items():
            if len(data['flights']) >= min_flights:
                ...
                avg_heading = self._circular_mean(data['headings']) if data['headings'] else None
        ...
        for corridor in corridors[:10]:
            print(f"  #{corridor['rank']:2d}: ({corridor['center_lat']:.4f}, {corridor['center_lon']:.4f}) - "
    ...
    def _get_grid_cell(self, lat: float, lon: float, grid_size_km: float) -> Tuple[float, float]:
