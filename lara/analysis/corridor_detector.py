...
class CorridorDetector:
    ...
    def detect_corridors(
        ...
        print(f"   Parameters: min_flights={min_flights}, "
        ...
        print(f"   Fitted {len(corridors)} corridors, "
    ...
    def _fit_line_least_squares(
        ...
        if use_lat_as_independent:
            ...
            numerator = sum((lats[i] - mean_lat) * (lons[i] - mean_lon) 
                           for i in range(len(lats)))
            denominator = sum((lats[i] - mean_lat) ** 2 
                             for i in range(len(lats)))
            ...
            numerator = sum((lons[i] - mean_lon) * (lats[i] - mean_lat) 
                           for i in range(len(lats)))
            denominator = sum((lons[i] - mean_lon) ** 2 
