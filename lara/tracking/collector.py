...
class FlightCollector:
        ...
        try:
            ...
            if self.auth:
                response = self.auth.make_authenticated_request(
                    self.api_url, params=params, timeout=self.api_timeout
                )
            else:
            ...
            if response.status_code == 429:
                ...
                try:
                    ...
                    if response.status_code == 429:
                        print(
                            "   Still rate limited after waiting. Skipping this scan."
                        )
                        return []
                except Exception as e:
            ...
            if e.response.status_code == 401:
                print(
                    "❌ Authentication failed (401): Your OAuth2 credentials may be invalid, check your credentials.json file"
                )
                return []
    ...
    def process_flight(self, state: list, timestamp: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                "callsign": callsign,
                "distance": distance,
                "altitude": state_data.get("baro_altitude")
                or state_data.get("geo_altitude"),
    ...
    def print_header(self):
        ...
        print("=" * 70)
        print(
            f"Location:   {self.home_lat}°N, {self.home_lon}°E, {self.config.location_name}"
        )
        ...
        if self.auth:
            ...
            print(
                "Auth:       Anonymous (for higher limits, set up OAuth2 credentials)"
            )
        ...
        if self.update_interval < 15 and not self.auth:
            ...
            print(
                "   Set up OAuth2: Download credentials.json from opensky-network.org"
            )
            print("=" * 70)
    ...
    def run_single_iteration(self) -> int:
        ...
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}] Scan #{self.iteration_count}...",
            end=" ",
        )
        ...
        if not flights:
            ...
            if self.consecutive_empty_scans == 1:
                ...
                print(
                    f"No flights detected ({self.consecutive_empty_scans} consecutive)"
                )
            ...
            while True:
                ...
                if self.iteration_count % 10 == 0:
                    try:
                        stats = self.db.get_statistics()
                        empty_rate = (
                            self.total_empty_scans / self.iteration_count
                        ) * 100
    ...
    def _handle_shutdown(self):
        ...
        try:
            ...
            print(
                f"   Empty scans: {self.total_empty_scans:,} ({(self.total_empty_scans/max(self.iteration_count, 1)*100):.1f}%)"
            )
