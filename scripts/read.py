...
from tracking import FlightReader, Config
...
def display_overview(reader: FlightReader):
    ...
    if stats['avg_altitude_m']:
        ...
        print(f"Average Altitude:         {stats['avg_altitude_m']:.0f} m ({avg_alt_ft:.0f} ft)")
...
def display_recent_flights(reader: FlightReader):
    ...
    for flight in flights:
        ...
        if flight['max_altitude_m'] and flight['min_altitude_m']:
            alt_range = f"{flight['min_altitude_m']:.0f}-{flight['max_altitude_m']:.0f}m"
        else:
            alt_range = 'N/A'
...
def display_countries(reader: FlightReader):
    ...
    for country in countries:
        print(f"{country['origin_country']:<25} {country['flight_count']:<12} "
